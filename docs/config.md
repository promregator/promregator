...
The default value of this options is "true".
...
If this option is set to "true" and Single Target Scraping is used, then - even though the necessary information is available via the "file_sd_configs" metadata - label enrichment takes place for all metrics returned by Promregator. Due to historical reasons this is the case.
...
Note that the metrics generated by Promregator itself are not still enriched by corresponding labels.
