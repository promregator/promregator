...
public class AutoRefreshingCacheMapTest {
	...
	public void testPutAndGet() {
		AutoRefreshingCacheMap<String, String> subject = new AutoRefreshingCacheMap<>(Duration.ofSeconds(1), Duration.ofSeconds(1), key -> {
	...
	public void testExpiry() throws InterruptedException {
		AutoRefreshingCacheMap<String, String> subject = new AutoRefreshingCacheMap<>(Duration.ofMillis(100), Duration.ofSeconds(10), key -> {
			char[] source = key.toCharArray();
			char[] target = new char[source.length];
			for (int i = 0;i<source.length;i++) {
				target[source.length-i-1] = source[i];
	...
	public void testAutoRefresh() throws InterruptedException {
		...
		AutoRefreshingCacheMap<String, String> subject = new AutoRefreshingCacheMap<>(Duration.ofSeconds(10), Duration.ofMillis(500), key -> {
			counter.increase();
			return "refreshed";
		});
	...
	public void testMassRun() throws InterruptedException {
		AutoRefreshingCacheMap<String, String> subject = new AutoRefreshingCacheMap<>(Duration.ofMillis(200), Duration.ofMillis(200), key -> {
			char[] source = key.toCharArray();
			char[] target = new char[source.length];
			for (int i = 0;i<source.length;i++) {
				target[source.length-i-1] = source[i];
